...
Test(sfmm_basecode_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	sf_errno = 0;
	sf_malloc(25);

	double exp_fragmentation = (double)25 / 48;
	double found = sf_fragmentation();

	cr_assert(exp_fragmentation == found, "fragmentation should be %f", exp_fragmentation);
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
}
...
Test(sfmm_basecode_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	...
	double exp_utilization = (double)25 / 4096;
	double found = (double)sf_utilization();

	cr_assert(exp_utilization == found, "utilization should be %f", exp_utilization);
	cr_assert(sf_errno == 0, "sf_errno is not zero!");	
}
...
Test(sfmm_basecode_suite, student_test_5, .timeout = TEST_TIMEOUT) {
	...
	assert_free_block_count(3872, 1);
}